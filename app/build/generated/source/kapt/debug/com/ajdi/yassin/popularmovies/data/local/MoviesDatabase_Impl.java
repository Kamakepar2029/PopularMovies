...
import com.ajdi.yassin.popularmovies.data.local.dao.CastsDao_Impl;
import com.ajdi.yassin.popularmovies.data.local.dao.MoviesDao;
import com.ajdi.yassin.popularmovies.data.local.dao.MoviesDao_Impl;
import com.ajdi.yassin.popularmovies.data.local.dao.ReviewsDao;
import com.ajdi.yassin.popularmovies.data.local.dao.ReviewsDao_Impl;
import com.ajdi.yassin.popularmovies.data.local.dao.TrailersDao;
import com.ajdi.yassin.popularmovies.data.local.dao.TrailersDao_Impl;
...
public final class MoviesDatabase_Impl extends MoviesDatabase {
  ...
  protected SupportSQLiteOpenHelper createOpenHelper(DatabaseConfiguration configuration) {
    final SupportSQLiteOpenHelper.Callback _openCallback = new RoomOpenHelper(configuration, new RoomOpenHelper.Delegate(1) {
      ...
      public void createAllTables(SupportSQLiteDatabase _db) {
        _db.execSQL("CREATE TABLE IF NOT EXISTS `movie` (`originalLanguage` TEXT, `id` INTEGER NOT NULL, `title` TEXT, `poster_path` TEXT, `backdrop_path` TEXT, `overview` TEXT, `popularity` REAL NOT NULL, `vote_average` REAL NOT NULL, `vote_count` INTEGER NOT NULL, `release_date` TEXT, `is_favorite` INTEGER NOT NULL, `genres` TEXT, PRIMARY KEY(`id`))");
      ...
      protected void validateMigration(SupportSQLiteDatabase _db) {
        ...
        if (! _infoMovie.equals(_existingMovie)) {
          throw new IllegalStateException("Migration didn't properly handle movie(com.ajdi.yassin.popularmovies.data.local.model.Movie).\n"
                  + " Expected:\n" + _infoMovie + "\n"
                  + " Found:\n" + _existingMovie);
        }
        ...
        if (! _infoTrailer.equals(_existingTrailer)) {
          throw new IllegalStateException("Migration didn't properly handle trailer(com.ajdi.yassin.popularmovies.data.local.model.Trailer).\n"
                  + " Expected:\n" + _infoTrailer + "\n"
                  + " Found:\n" + _existingTrailer);
        }
        ...
        if (! _infoCast.equals(_existingCast)) {
          throw new IllegalStateException("Migration didn't properly handle cast(com.ajdi.yassin.popularmovies.data.local.model.Cast).\n"
                  + " Expected:\n" + _infoCast + "\n"
                  + " Found:\n" + _existingCast);
        }
        ...
        if (! _infoReview.equals(_existingReview)) {
          throw new IllegalStateException("Migration didn't properly handle review(com.ajdi.yassin.popularmovies.data.local.model.Review).\n"
                  + " Expected:\n" + _infoReview + "\n"
                  + " Found:\n" + _existingReview);
        }
      }
    }, "096304796c856f42f200303d71d41096", "ef365d0ff254c2a4eb084e6316dec1af");
  ...
  protected InvalidationTracker createInvalidationTracker() {
    ...
    return new InvalidationTracker(this, _shadowTablesMap, _viewTables, "movie","trailer","cast","review");
  }
